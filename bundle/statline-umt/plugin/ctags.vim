...
autocmd BufEnter *.c,*.cpp,*.h,*.py,*.vim,*.java
...
autocmd BufLeave *.c,*.cpp,*.h,*.py,*.vim,*.java
...
"autocmd CursorHold *
"\   if exists('g:CTAGS_updatestatline') && g:CTAGS_updatestatline != 0
...
" corresponding tag.
function! StatlineGetTagName(curline)
    if !exists("b:lines")
	return ""
    endif
