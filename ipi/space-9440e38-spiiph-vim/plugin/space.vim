...
if !exists("g:space_no_search") || !g:space_no_search
    ...
    if maparg('#', 'v') != ''
        nnoremap <expr> <silent> # <SID>setup_space("searchreverse", "#")
        onoremap <expr> <silent> # <SID>setup_space("searchreverse", "#")
    else
        noremap  <expr> <silent> # <SID>setup_space("searchreverse", "#")
    endif

    noremap <expr> <silent> g* <SID>setup_space("search", "g*")
    noremap <expr> <silent> g# <SID>setup_space("searchreverse", "g#")
...
function! s:setup_space(type, command)
    ...
    elseif a:type == "search"
        let s:space_move = "nzz"
        let s:shift_space_move = "Nzz"
        let s:cmd_type = "search"
        let cmd = <SID>maybe_open_fold(cmd)
    elseif a:type == "searchreverse"
        let s:space_move = "Nzz"
        let s:shift_space_move = "nzz"
        let s:cmd_type = "searchreverse"
        let cmd = <SID>maybe_open_fold(cmd)
    elseif a:type == "cjump"
        let s:space_move = "g,"
        ...
        let cmd = <SID>maybe_open_fold(cmd)
    elseif a:type == "jump"
        let s:space_move = "\<C-i>zz"
        let s:shift_space_move = "\<C-o>zz"
        let s:cmd_type = "jump"
        let cmd = <SID>maybe_open_fold(cmd)
    elseif a:type == "tag"
        let s:space_move = "tn"
